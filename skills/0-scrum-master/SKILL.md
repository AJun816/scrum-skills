...
**当用户发送任务时，Scrum Master自动执行全自动化敏捷开发流程：**

- 用户只需描述需求（例如："开发一个用户登录功能"）
- 自动分析任务类型（新功能/Bug修复/架构优化/需求变更）
- 自动创建任务列表（使用TaskCreate）
- 自动分配任务给团队成员（使用TaskUpdate）
- 团队成员并行工作，自动协作
- 实时监控进度，自动检测阻塞
- 自动进行代码质量检查
- 输出测试报告
...
1. ✅ **全自动化** - 用户只需描述需求，自动执行完整流程
2. ✅ **智能协调** - 自动识别任务类型，分配合适的团队成员
3. ✅ **实时监控** - 每30秒监控进度，自动检测阻塞
4. ✅ **并行工作** - 支持多个技能实例并行工作，最大化效率
5. ✅ **质量保障** - 自动进行代码质量检查，确保交付质量
6. ✅ **持续改进** - 促进团队自组织和主动协作
