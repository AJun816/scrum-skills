...
2. 识别项目使用的框架（Vue/React/Angular/Svelte等），适配对应的开发规范
3. 确认需求对应的**后端API接口**：
   - 查看 `PROJECT_CONFIG.md` 中的 API端点概览和业务域定义
   - 定位后端 API 路径和请求/响应格式
   - 确认 API 请求方法(GET/POST/PUT/DELETE)、参数格式、响应结构
4. 定位需求所属的**功能模块**
5. 检查是否存在可复用的现有代码（组件、hooks/composables、API函数）
...
1. 确保无语法错误
2. 检查 import 路径正确性
3. 确认 UI 组件库使用正确（props/events/slots）
