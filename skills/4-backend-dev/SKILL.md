...
3. 识别项目使用的语言和框架（Java/Go/Python/Node.js等），适配对应的开发规范
4. 定位需求所属的**业务域**（根据 PROJECT_CONFIG.md 中的 business_domains）
5. 确认需求涉及的**分层**（根据 PROJECT_CONFIG.md 中的架构模式）
6. 检查是否存在可复用的现有代码（避免重复实现）
7. **复用扫描**：使用 Grep 搜索项目中是否已有类似功能的 Service/Repository/Utils，优先复用
...
1. **代码自检**（强制执行，不依赖外部审查）：
   ...
   - ❌ **禁止提交**：存在与项目已有实现重复的代码
   - ❌ **禁止提交**：包含硬编码密钥、SQL 拼接
   - ❌ **禁止提交**：残留调试语句（console.log/print/debug）
   - ❌ **禁止提交**：存在冗余 import 或循环依赖
2. **Lint 检查循环**（强制执行，不可跳过）：
   - 运行项目 Linter（根据技术栈：eslint/ruff/flake8/go vet/checkstyle 等）
   - 如有 error 级别问题，立即修复
   - 修复后重新运行 Linter，直到零 error
   - 连续 3 轮仍有 error → 停止并报告问题，禁止强行提交
3. **关联测试验证**（强制执行，不可跳过）：
   - 查找与变更代码相关的测试文件（src/test/ 或 tests/ 下同模块测试）
   - 如存在关联测试，运行测试（mvn test / gradle test / pytest / go test 等）
   - 测试失败时修复代码，重新运行，直到全部通过
   - 如无关联测试文件，跳过并在提交信息中注明
   - ❌ 禁止提交导致已有测试失败的代码
4. 读取生成的文件，检查分层约束和命名规范
